...
export default function AdminPanel() {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8" id="admin-panel-container">
      ...
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 pb-5 mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900 font-sans flex items-center gap-2">
            <Settings className="h-6 w-6 text-gray-700 animate-spin-slow" />
            <span>Storefront Administrator Control Panel</span>
          </h1>
          <p className="text-xs text-gray-500 mt-1">Manage global inventory catalog, stock limits, and customer fulfillment logs.</p>
        </div>
      </div>

      {/* Notifications */}
      {errorMsg && (
        <div className="mb-6 flex items-center gap-2 p-3.5 rounded-xl bg-red-50 border border-red-100 text-red-700 text-xs font-semibold">
          <ShieldAlert className="h-4 w-4 flex-shrink-0" />
          <span>{errorMsg}</span>
        </div>
      )}
      {successMsg && (
        <div className="mb-6 flex items-center gap-2 p-3.5 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs font-semibold">
        ...
        <div className="lg:col-span-2 space-y-6" id="admin-inventory-section">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors cursor-pointer"
          ...
          {/* New Product Form */}
          {isAdding && (
            <form onSubmit={handleAddProduct} className="p-5 bg-gray-50/50 border border-gray-150 rounded-xl space-y-4" id="new-product-form">
              <h3 className="text-xs font-bold text-gray-900 font-mono uppercase">Launch New Premium Asset</h3>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase font-mono">Product Name</label>
                  <input
                    type="text"
                    required
                    value={newProduct.name}
                    onChange={(e) => setNewProduct({ ...newProduct, name: e.target.value })}
                    className="mt-1 w-full text-xs rounded-lg border border-gray-200 p-2.5 bg-white focus:outline-hidden focus:border-gray-900 focus:ring-1 focus:ring-gray-900"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 w-full text-xs rounded-lg border border-gray-200 p-2.5 bg-white focus:outline-hidden focus:border-gray-900"
                  >
              ...
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase font-mono">Full Description</label>
                <textarea
                  rows={3}
                  required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase font-mono">Price ($ USD)</label>
                  <input
                    type="number"
                    step="0.01"
                    required
                ...
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase font-mono">Initial Stock</label>
                  <input
                    type="number"
                    required
              ...
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase font-mono">Asset Image URL</label>
                <input
                  type="url"
                  required
              ...
              <div className="flex justify-end gap-3 pt-3">
                <button
                  ...
                  className="px-4 py-2 border border-gray-200 text-gray-700 font-bold text-xs rounded-lg hover:bg-gray-100 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-gray-900 text-white font-bold text-xs rounded-lg hover:bg-gray-800 transition-colors cursor-pointer"
                >
                  Publish Product
                </button>
              </div>
          ...
          {/* Products List Table */}
          <div className="bg-white rounded-xl border border-gray-150 overflow-hidden" id="admin-products-table">
            {loadingProducts ? (
              <div className="py-8 text-center text-xs text-gray-400 animate-pulse">Syncing catalog inventory logs...</div>
            ) : products.length === 0 ? (
              <div className="py-12 px-4 text-center space-y-4">
                <div className="text-xs text-gray-400">The storefront inventory catalog is currently empty.</div>
                <button
              ...
              <div className="divide-y divide-gray-100">
                {products.map((p) => (
                  <div key={p.id} className="p-4 flex items-center justify-between gap-4 flex-wrap hover:bg-gray-50/50 transition-colors">
                    <div className="flex items-center gap-3">
                      <img src={p.image} alt={p.name} className="h-10 w-10 object-cover rounded-lg border border-gray-100 bg-gray-50" />
                      <div>
                        <span className="text-xs font-bold text-gray-900 line-clamp-1">{p.name}</span>
                        <span className="text-[10px] text-gray-400 mt-0.5 block">{p.category} • <span className="font-mono">${p.price}</span></span>
                      </div>
                    </div>

                    <div className="flex items-center gap-4">
                      {/* Stock Adjuster */}
                      <div className="flex items-center gap-1 bg-gray-50 p-1 rounded-lg border border-gray-100 text-xs">
                        <button
                          onClick={() => handleUpdateStock(p.id, p.stock, -1)}
                          disabled={p.stock <= 0}
                          className="h-6 w-6 rounded-md hover:bg-white flex items-center justify-center font-bold text-gray-500 disabled:opacity-30 cursor-pointer"
                        >
                          -
                        </button>
                        <span className="px-2 font-bold font-mono text-gray-800 min-w-[20px] text-center">{p.stock}</span>
                        <button
                          onClick={() => handleUpdateStock(p.id, p.stock, 1)}
                          className="h-6 w-6 rounded-md hover:bg-white flex items-center justify-center font-bold text-gray-500 cursor-pointer"
                        >
                          +
                        </button>
                      </div>
        ...
        <div className="space-y-6" id="admin-orders-section">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1" id="admin-orders-scroll">
            {loadingOrders ? (
              ...
              <div className="py-8 text-center text-xs text-gray-400 border border-dashed border-gray-150 rounded-xl bg-gray-50/20">
                No active sales orders.
              </div>
            ) : (
              orders.map((order) => (
                <div key={order.id} className="p-4 bg-white border border-gray-150 hover:border-gray-350 rounded-xl space-y-3 transition-colors text-xs text-left">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-gray-900 font-mono">#{order.id?.slice(-5).toUpperCase()}</span>
                  ...
                  <div className="flex justify-between items-center text-2xs pt-1 border-t border-gray-50 text-gray-500">
                    <span>{order.items.length} unique items</span>
                    <span className="font-bold text-gray-900 font-mono">${order.total.toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-2" id={`actions-${order.id}`}>
                    {order.status === 'Pending' && (
                      <button
                        ...
                        className="px-2.5 py-1 bg-amber-50 border border-amber-200 text-amber-700 font-bold text-[9px] rounded-md hover:bg-amber-100 transition-colors cursor-pointer"
                      >
                        Accept & Process
                      </button>
                    )}
                    {order.status === 'Processing' && (
                      <button
                        ...
                        className="px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-700 font-bold text-[9px] rounded-md hover:bg-blue-100 transition-colors cursor-pointer"
                      >
                        Dispatch (Ship)
                      </button>
                    )}
                    {order.status === 'Shipped' && (
                      <button
                        ...
                        className="px-2.5 py-1 bg-emerald-50 border border-emerald-200 text-emerald-700 font-bold text-[9px] rounded-md hover:bg-emerald-100 transition-colors cursor-pointer"
                      >
                        Mark Delivered
                      </button>
                    )}
                    <span className="text-[10px] text-gray-500 font-bold px-2 py-0.5 bg-gray-50 rounded border border-gray-100 ml-auto">
                      {order.status}
                    </span>
                  </div>
                </div>
